...
function atualizarEvento(EventoAtualizar: Evento): void {
    ...
    if (index === -1) {
        throw new Error(`Evento ${EventoAtualizar.nome} não foi encontrado para edição.`);
    }
    ...
    console.log(`O evento ${EventoAtualizar.nome} foi editado com sucesso!`);
...
function  cancelarEvento(existente: IEvento) {      
      ...
      if(eventoExistente){
        console.log(`Nenhum evento com ID "${existente.id}"  encontrado.`);
    } else {
        salvar(p);
        console.log(` Evento com ID "${existente.id}" cancelado com sucesso.`);
    }
